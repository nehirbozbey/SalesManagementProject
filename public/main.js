...
async function loadDashboardData() {
  try {
    ...
    document.getElementById("daily-applications").textContent =
      stats.daily_applications;
...
async function loadCommissionData() {
  try {
    const response = await fetch(`${API_BASE_URL}/commission/rates`);
...
async function loadApplicationData() {
  try {
    ...
    document.getElementById("rejected-applications").textContent =
      stats.rejected;
    document.getElementById("application-revenue").textContent =
      `₺${stats.total_revenue.toLocaleString()}`;
...
async function loadDemographyData() {
  try {
    const regionalResponse = await fetch(
      `${API_BASE_URL}/demographics/regional`,
    );
...
async function loadSectorData() {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/sectors/analysis?${new URLSearchParams(filters)}`,
    );
    const data = await response.json();
...
async function loadProductData() {
  try {
    const missingResponse = await fetch(
      `${API_BASE_URL}/products/missing-analysis`,
    );
    const missingData = await missingResponse.json();

    const sectorResponse = await fetch(
      `${API_BASE_URL}/products/sector-analysis`,
    );
...
function getActiveFilters() {
  return {
    ...
    sectors: Array.from(document.querySelector("#sectorFilter").selectedOptions)
      .map((opt) => opt.value)
      .join(","),
    regions: Array.from(document.querySelector("#regionFilter").selectedOptions)
      .map((opt) => opt.value)
      .join(","),
  };
}
